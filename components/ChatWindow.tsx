...
const ChatWindow: React.FC<ChatWindowProps> = ({ chat, settings, allPersonas, onUpdateChat, onDeleteChat, onAddToFavorites, onBack }) => {
  ...
  return (
    <div className="flex-1 h-full flex flex-col bg-[#f5f5f5] relative">
      ...
      >
        {messages.length === 0 && !isReadOnly && !isProcessing ? (
             <div className="h-full flex flex-col items-center justify-center text-gray-500">
                <div className="w-16 h-16 bg-gray-200 rounded-full flex items-center justify-center mb-4">
                     <svg className="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                </div>
                <h3 className="text-lg font-medium text-gray-700 mb-2">开始新的群聊</h3>
                <p className="text-sm text-gray-400 mb-6 text-center max-w-xs">
                    当前暂无成员。请先配置 AI 角色和发言规则。
                </p>
                
                <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm text-sm space-y-4 max-w-sm w-full mx-4">
                     <div className="flex items-center gap-3">
                         <div className="w-6 h-6 rounded-full bg-green-100 text-green-600 flex items-center justify-center font-bold text-xs flex-shrink-0">1</div>
                         <span className="text-gray-600">点击右上角 <span className="font-bold text-gray-800">设置图标</span></span>
                     </div>
                     <div className="flex items-center gap-3">
                ...
                <button 
                    onClick={() => setIsChatSettingsOpen(true)}
                    className="mt-8 px-6 py-2.5 bg-[#07c160] hover:bg-[#06ad56] text-white rounded-full font-medium transition-colors shadow-lg shadow-green-100 flex items-center gap-2"
                >
